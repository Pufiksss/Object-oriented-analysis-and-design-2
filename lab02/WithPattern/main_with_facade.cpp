...
public:
    std::vector<Book> getLibrary() {
        return db_.getLibrary();
    }

    Book addBook(const std::string& url) {
        std::optional<Book> maybeBook = driver_.getBook(url);
        if (!maybeBook.has_value())
            throw std::runtime_error("Не удалось получить данные книги");
        return db_.addBook(maybeBook.value());
    }

    bool removeBook(int index) {
        return db_.removeBook(index);
    }

    std::vector<Book> search(const std::string& query) {
        return db_.searchBooks(query);
    }
...
private slots:
    void loadLibrary() {
        bookList_->clear();
        for (const auto& book : dibooks_.getLibrary())
            bookList_->addItem(
    ...
    void onAddClicked() {
        ...
        try {
            Book added = dibooks_.addBook(url);
            bookList_->addItem(
    ...
    void onDeleteClicked() {
        ...
        bool ok = dibooks_.removeBook(row);
        if (ok) {
    ...
    void onSearchClicked() {
        ...
        std::vector<Book> found = dibooks_.search(query);
    ...
    void updateStatus() {
        statusLabel_->setText(QString("Всего книг: %1").arg(dibooks_.count()));
    }
};
