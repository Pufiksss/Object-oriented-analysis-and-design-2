...
public class GraphicEditor
{
    ...
    public ShapeModel? DuplicateShape(ShapeModel original)
    {
        ...
        if (original is TriangleModel t)
    ...
    public ShapeModel? HitTest(double x, double y)
    {
        ...
        {
            var s = _shapes[i];
            if (s is CircleModel c &&
                Math.Sqrt(Math.Pow(x - c.X, 2) + Math.Pow(y - c.Y, 2)) <= c.Radius)
                return c;
            if (s is RectModel r &&
                x >= r.X && x <= r.X + r.W && y >= r.Y && y <= r.Y + r.H)
                return r;
            if (s is TriangleModel t &&
...
public class DrawingCanvas : Control
{
    ...
    public override void Render(DrawingContext ctx)
    {
        ...
        var ft = new FormattedText(
            ...
            new Typeface("Arial"),
            13, new SolidColorBrush(Color.FromArgb(90, 255, 255, 255)));
        ctx.DrawText(ft, new Point(Bounds.Width - 90, 8));
    }

    private void DrawShape(DrawingContext ctx, ShapeModel s)
    {
        var fill = new SolidColorBrush(s.FillColor);
        var penColor = s.IsSelected ? Color.FromRgb(50, 180, 255) : s.BorderColor;
        var pen = new Pen(new SolidColorBrush(penColor), s.IsSelected ? 3 : 1.5);

        if (s is CircleModel c)
        {
            ctx.DrawEllipse(fill, pen,
                new Point(c.X, c.Y), c.Radius, c.Radius);
            if (c.IsSelected)
                DrawHandles(ctx, c.X - c.Radius, c.Y - c.Radius, c.Radius * 2, c.Radius * 2);
        }
        else if (s is RectModel r)
        {
            ctx.FillRectangle(fill, new Rect(r.X, r.Y, r.W, r.H));
            ctx.DrawRectangle(null, pen, new Rect(r.X, r.Y, r.W, r.H));
            if (r.IsSelected)
                DrawHandles(ctx, r.X, r.Y, r.W, r.H);
        }
        else if (s is TriangleModel t)
        {
            ...
            {
                gc.BeginFigure(new Point(t.X + t.W / 2, t.Y), true);
                gc.LineTo(new Point(t.X + t.W, t.Y + t.H));
                gc.LineTo(new Point(t.X, t.Y + t.H));
                gc.EndFigure(true);
            }
            ctx.DrawGeometry(fill, pen, geo);
            if (t.IsSelected)
                DrawHandles(ctx, t.X, t.Y, t.W, t.H);
        }
    }

    private void DrawHandles(DrawingContext ctx, double x, double y, double w, double h)
    {
        var pts = new Point[]
        {
        ...
        var hPen = new Pen(new SolidColorBrush(Color.FromRgb(50, 180, 255)), 1);
        foreach (var p in pts)
            ctx.FillRectangle(hFill, new Rect(p.X - 4, p.Y - 4, 8, 8));
        foreach (var p in pts)
            ctx.DrawRectangle(null, hPen, new Rect(p.X - 4, p.Y - 4, 8, 8));
    }
}
...
public class MainWindow : Window
{
    ...
    private void BuildUI()
    {
        ...
        toolbar.Children.Add(MakeLabel("⬡  ИНСТРУМЕНТЫ", Color.FromRgb(100, 160, 255)));
        ...
        toolbar.Children.Add(MakeActionBtn("⧉  Дублировать  [D]", DoDuplicate, Color.FromRgb(40, 80, 40)));
        toolbar.Children.Add(MakeActionBtn("✕  Удалить  [Del]", DoDelete, Color.FromRgb(80, 30, 30)));
        toolbar.Children.Add(MakeActionBtn("↺  Очистить всё", DoClear, Color.FromRgb(50, 50, 60)));
        toolbar.Children.Add(MakeSep());
        toolbar.Children.Add(MakeLabel("ЦВЕТ ЗАЛИВКИ", Color.FromRgb(100, 160, 255)));
    ...
    private Button MakeToolBtn(string text, string toolName)
    {
        var btn = new Button
        {
            ...
            Foreground = new SolidColorBrush(Color.FromRgb(210, 210, 230)),
